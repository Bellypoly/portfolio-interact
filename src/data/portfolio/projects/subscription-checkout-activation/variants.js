  ...
  "data-reporter": Object.freeze({
    ...
    earlyImpactGroups: [
      ...
      },
      {
        ...
        rows: [
          { value: "+7%", label: "Conversion uplift" },
          { value: "+9%", label: "Member information completion" },
          { value: "+22%", label: "Payment details completed" },
        ],
      },
    ],
    ...
    problemSection: {
      ...
      listGroups: [
        ...
        {
          title: "Where the platform fought back",
          items: [
      ...
      diagram: {
        ...
        intro:
          "The screenshots above show what readers experienced. The diagram below shows what I engineered against: how many page boundaries sat between intent and payment, and what happened to state at each one. Version 1 fragmented the funnel across 3 navigations with no shared context. Version 2 put everything on one surface with a linear path through the same stages — and kept client state alive the entire time.",
        beforeAfter: true,
    ...
    checkoutSection: {
      ...
      bullets: [
        "Moved member info, payment, and order summary onto a single page — eliminating two full navigations and two server round-trips. Client state now lived in one component tree instead of being serialized and hoped to survive across page loads.",
        "Built a unified payment abstraction over Braintree, Stripe, and Apple Pay. Each provider had its own tokenization flow, validation contract, and error shape; the abstraction normalized all of that behind one interface so the checkout form did not care which provider was active.",
    ...
    claritySection: {
      ...
      figureColumns: 1,
      bullets: [
        "The progress bar had to go. Users' mental model was tab navigation; our implementation was one long form with server-side ordering constraints. Those two models couldn't coexist. I removed it and replaced it with prominent section headers that described the flow without implying you could jump around. The markup stopped lying about how the form worked.",
        "The payment gating was harder to let go of. I had built it deliberately: Arc XP required validated member info before creating a subscription order, so I disabled payment fields until that check passed. Architecturally clean. But Clarity showed readers interpreting disabled inputs as broken UI — and abandoning. The fix: decouple presentation from validation. Payment fields rendered active on first paint; order creation still waited for valid member data before it fired. Same backend safety, honest frontend.",
        ...
        "Tightened validation end to end: specific inline messages, immediate feedback, and recovery paths — no more generic errors that forced a full retry.",
      ],
      frictionCompare: {
        caption:
      ...
      beforeAfterDiagram: {
        ...
        caption:
          "Apples to apples: 1 page before and after. V3 removed the progress bar, un-gated payment fields, and tucked benefits behind a toggle — each change tied to the session data above.",
      },
    },

    onboardingSection: {
      title: "Making the first week count",
      lead: "A subscription is a recurring charge. If someone forgets why they're paying before the first renewal, they cancel. The checkout system knew exactly who the new subscriber was, what plan they picked, and what they had seen on the way in. I piped that context into a 3-step onboarding sequence that fired the moment payment confirmed — while intent was still hot.",
      stepBullets: [
        "Newsletter selection — because the subscriber was already authenticated, we could pre-filter the catalog to exclude newsletters they already received. The first email felt personal because the system did the work, not the reader.",
        "App adoption — a direct prompt to install the mobile app with deep links to both stores. App users have significantly higher session frequency and retention; this step was the biggest churn lever we could influence at onboarding time.",
        "Product discovery — a curated tour of content verticals, tools, and features many subscribers do not know exist, including events, podcasts, and puzzles. This anchored the perceived value of the subscription beyond just articles and gave readers reasons to return that were not tied to a single news cycle.",
      ],
      figures: [
        {
    ...
    impactAfterV3: {
      ...
      figureCaption:
        "Source: The Dallas Morning News employee town hall, 13 August 2025.",
      rows: [
        { value: "+7%", label: "Conversion uplift (Clarity iteration)" },
        { value: "+9%", label: "Member information completion" },
        { value: "+22%", label: "Credit card / payment details completed" },
      ],
    },
